...
Tested behaviours:
  ...
  - on_system_stop: does NOT publish tcp.session.closed (only _on_session_closed does)
...
class TestStartServer:
    ...
    def test_server_started_publishes_host_and_port(self):
        _MockTCPServer.return_value  = _make_server()
        _MockFrameRelay.return_value = MagicMock()
        tcp._start_server()
    ...
    def test_accept_timeout_publishes_error(self):
        _MockTCPServer.return_value = _make_server(accept_result=None)
        tcp._start_server()
        assert len(_published("tcp.server.error")) == 1

    def test_session_connected_published_after_accept(self):
        _MockTCPServer.return_value  = _make_server(accept_result=(MagicMock(), "192.168.50.100"))
        _MockFrameRelay.return_value = MagicMock()
        tcp._start_server()
...
class TestOnFrame:
    ...
    def test_empty_payload_publishes_empty_hex(self):
        ...
        assert _published("aa.frame.received")[0]["payload_hex"] == ""
...
class TestSystemStop:
    ...
    def test_does_not_publish_session_closed(self):
        ...
        tcp.on_system_stop("system.stop", {})
        assert len(_published("tcp.session.closed")) == 0
