...
_MSG_PAIRING_REQUEST       = 0x8001
_MSG_PAIRING_RESPONSE      = 0x8002
...
class BluetoothChannelHandler:
    ...
    async def handle_frame(self, channel_id: int, message_id: int, body: bytes) -> None:
        self.log.info(f"BluetoothChannel (ch{channel_id}) msgId=0x{message_id:04x} len={len(body)}")
        ...
        elif message_id == _MSG_PAIRING_REQUEST:
            # Pairing response (already paired = True, status = STATUS_OK)
            resp = b"\x08\x00\x10\x01"
            await self.manager.send_wire_frame(channel_id, _MSG_PAIRING_RESPONSE, resp, encrypted=True)
