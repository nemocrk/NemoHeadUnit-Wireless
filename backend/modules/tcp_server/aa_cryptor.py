...
Owner: tcp_server module.
  - tcp_server drives the TLS handshake via bus messages from oaa_control_channel
  - tcp_server decrypts incoming encrypted frames before publishing to the bus
  - tcp_server encrypts outgoing frames when encryptionType=Encrypted is required
...
log = get_logger("tcp_server.aa_cryptor")
...
class AACryptor:
    ...
    Owned by tcp_server — drives TLS handshake rounds triggered via bus
    messages from oaa_control_channel, then handles encrypt/decrypt of
    all subsequent encrypted frames autonomously.
